...
parser.add_argument('-nm', action = 'store', dest = 'nm', type = int, help='Interval of data reading in minutes.', default = 5)
parser.add_argument('-ns', action = 'store', dest = 'ns', type = int, help='Interval of down-sampling in minutes.', default = 1)
...
ns = args.ns; print('Downsampling every '+str(ns)+' min.')
...
for t_ in np.arange(t1, t2, nm*60):
    
    t1_ = t_
    t2_ = t1_+60*nm
    
    print("Loading.. %.1f"%(100*(t2_-t1)/(t2-t1))+'%')
    
    data_bfield = mydb.get([PS_bfield_variables()[var] for var in PS_bfield_variables()], t1_,t2_)
    data_pos_bpm = mydb.get([bpm_position_variables()[var] for var in bpm_position_variables()], t1_, t2_)
    data_cycle = mydb.get('LEI.LSA:CYCLE',t1_,t2_)
    data_bct =  mydb.get([bct_line_variables()[var] for var in bct_line_variables()], t1_, t2_)
    data_bct_ring =  mydb.get([bct_ring_variables()[var] for var in bct_ring_variables()], t1_, t2_)
    ...
    #df0 =df
    df = pd.concat([df,df_bct_new],axis=1)
    df.drop('ER.BCTDC', 1,inplace=True)
    ...
    df = df.resample(str(ns)+'T', on = 'date').mean()
    df_t.append(df)

df_t = pd.concat(df_t)
saveDir = '..//Monitoring/'
t1_str = str(df_t.index.min())
t2_str = str(df_t.index.max())
...
for n_inj in np.arange(N_inj):
	...
	for var in bct_line_variables():
		ax1.plot(df_t.index, df_t[var+'_%d'%n_inj].rolling(rw).mean().values, label = var)
	ax1.legend(bbox_to_anchor=(1.04,1), loc="upper left")
	ax1.set_xticklabels([])
	ax1.set_ylabel('N [1e10 c]')
	ax1.set_title(fundamental+', intensity of injection #%d overview'%n_inj)
	
	ax2.plot(df_t.index, 100*df_t['eff_%d'%n_inj].rolling(rw).mean().values,'-k')
	ax2.legend(['Efficiency'], loc = 0)
	ax2.set_ylabel('Inj. efficiency [%]')
	
	ax3 = ax2.twinx()
	ax3.plot(df_t.index, 100*df_t['trans_%d'%n_inj].rolling(rw).mean().values,'-r')
...
for n_inj in np.arange(N_inj):
	...
	ax3.plot(df_.index, 100*df_t['eff_%d'%n_inj].rolling(rw).mean().values)
...
for n_inj in np.arange(N_inj):
	...
	ax3.plot(df_t.index,df_t['PS_field_%d'%n_inj],'.-')
...
for var in bct_line_variables():
    print var
    c = color.next()
    s1.line(df_t.index, df_t[var+'_%d'%n_inj].rolling(rw).mean().values, color=c,line_width=2, alpha=0.8,
...
s2.line(df_t.index, 100*df_t['eff_%d'%n_inj].rolling(rw).mean().values,color='black',line_width=2, alpha=0.8,
...
s3.line(df_t.index, df_t['PS_field_%d'%n_inj].values,color='black',line_width=2, alpha=0.8,
...
for var in np.sort(df_.keys()):
    c = color.next()
    df_ = df_t.filter(regex='.*HOR_POS.*%d'%n_inj)
    s1.line(df_t.index, df_[var].rolling(rw).mean().values, color=c,line_width=2, alpha=0.8,
...
for var in np.sort(df_.keys()):
    c = color.next()
    s2.line(df_t.index, df_[var].rolling(rw).mean().values, color=c,line_width=2, alpha=0.8,
...
s3.line(df_t.index, 100*df_t['eff_%d'%n_inj].rolling(rw).mean().values, color='black',line_width=2, alpha=0.8,
